...
export default {
  ...
  computed: {
    ...
    sourceImgStyle() {
      ...
      const rotate = "rotate(" + scale.degree + "deg)";
      return {
        top,
        left,
        width: scale.width + "px",
        height: scale.height + "px",
        transform: rotate, // 旋转时 左侧原始图旋转样式
        "-ms-transform": rotate, // 兼容IE9
        "-moz-transform": rotate, // 兼容FireFox
        "-webkit-transform": rotate, // 兼容Safari 和 chrome
        "-o-transform": rotate, // 兼容 Opera
      };
    },
    // 原图蒙版属性
    sourceImgMasking() {
